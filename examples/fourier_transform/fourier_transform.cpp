...
public:
    ...
    template<typename F, 
             typename std::enable_if_t<std::is_same_v<T, float> && std::is_arithmetic_v<F> && !std::is_same_v<F, posit32_t>, int> = 0>
    Complex(F r, F i) {
...
inline Complex<posit32_t> multiplyComplexWithQuire(const Complex<posit32_t>& x, const Complex<posit32_t>& y) {
    posit32_t a = x.real;
    posit32_t b = x.imag;
    posit32_t c = y.real;
    posit32_t d = y.imag;
...
std::vector<RGB> visualizeError(const std::vector<RGB>& original, const std::vector<RGB>& reconstructed) {
    std::vector<RGB> errorImage(original.size());
    
    for (size_t i = 0; i < original.size(); ++i) {
        // Calculate absolute differences for each channel
        int rDiff = std::abs(static_cast<int>(original[i].r) - static_cast<int>(reconstructed[i].r));
        int gDiff = std::abs(static_cast<int>(original[i].g) - static_cast<int>(reconstructed[i].g));
        int bDiff = std::abs(static_cast<int>(original[i].b) - static_cast<int>(reconstructed[i].b));
        ...
        errorImage[i] = RGB(
            static_cast<unsigned char>(rDiff),
...
std::vector<Complex<T>> fft(const std::vector<Complex<T>>& spatial) {
    ...
    for (int k = 0; k < n / 2; ++k) {
        ...
        if constexpr (std::is_same_v<T, posit32_t>) {
            twiddle = Complex<T>(
                double_to_posit32(std::cos(angle)),
...
std::vector<Complex<posit32_t>> ifftWithQuire(const std::vector<Complex<posit32_t>>& frequency) {
    ...
    for (int i = 0; i < n; ++i) {
        conjugated[i] = Complex<posit32_t>(frequency[i].real, double_to_posit32(0) - frequency[i].imag);
    }
...
std::vector<RGB> createComparisonImage(
    ...
    for (int x = 0; x < size - 2; ++x) {
        comparisonImage[labelY * comparisonWidth + (size / 2 - originalLabel.length() * 3 + x)] = RGB(255, 255, 255);
        comparisonImage[labelY * comparisonWidth + (size + size / 2 - floatLabel.length() * 3 + x)] = RGB(255, 255, 255);
        comparisonImage[labelY * comparisonWidth + (2 * size + size / 2 - positLabel.length() * 3 + x)] = RGB(255, 255, 255);
        comparisonImage[labelY * comparisonWidth + (3 * size + size / 2 - quireLabel.length() * 3 + x)] = RGB(255, 255, 255);
    }
...
void processImageComparison() {
    ...
    auto floatDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endFloat - startFloat).count();
    ...
    auto positDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endPosit - startPosit).count();
    ...
    auto quireDuration = std::chrono::duration_cast<std::chrono::milliseconds>(endQuire - startQuire).count();
    ...
    std::vector<RGB> comparisonImage = createComparisonImage(
        originalImage, floatImage, positImage, quireImage, IMAGE_SIZE
    );
