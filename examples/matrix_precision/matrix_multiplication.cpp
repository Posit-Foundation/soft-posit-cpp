...
void initializeMatrices(std::vector<std::vector<T>> &A, std::vector<std::vector<T>> &B) {
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            ...
            if (signs(gen) > 0.5) val1 = -val1;
            if (signs(gen) > 0.5) val2 = -val2;
...
void generateHeatmapImage(const std::vector<std::vector<double>> &diff, 
    ...
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            if (diff[i][j] < minVal) minVal = diff[i][j];
            if (diff[i][j] > maxVal) maxVal = diff[i][j];
        }
    }
...
int main() {
    ...
    std::cout << "--------------------------------" << std::endl;
    std::cout << "Mean absolute difference between float and posit32: " << mean_diff_float_posit << std::endl;
    std::cout << "Mean absolute difference between float and posit32+quire: " << mean_diff_float_quire << std::endl;
    std::cout << "Mean absolute difference between posit32 and posit32+quire: " << mean_diff_posit_quire << std::endl;
    ...
    std::cout << "  - View the PPM files with any image viewer that supports the format" << std::endl;
