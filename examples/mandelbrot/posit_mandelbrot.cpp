...
struct ComplexP16 {
    ...
    ComplexP16 operator+(const ComplexP16& other) const {
        return ComplexP16(
            p16_add(real, other.real),
            p16_add(imag, other.imag)
        );
    }
...
int main() {
    ...
    for (int y = 0; y < HEIGHT; y++) {
        for (int x = 0; x < WIDTH; x++) {
            ...
            posit16_t imag = p16_add(yMin, p16_mul(p16_sub(yMax, yMin), p16_div(y_p16, height_p16)));
    ...
    std::cout << "Mandelbrot Set generated using p16 posits and saved to posit_mandelbrot.ppm" << std::endl;
    std::cout << "You can view this file using any image viewer that supports PPM format." << std::endl;
