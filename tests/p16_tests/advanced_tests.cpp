...
TEST(Posit16Advanced, MulDivCancel)
{
    ...
    {  // Reduced iteration count
        ...
        if (p_b.value == 0 || p_b.isNaR() || p_a.isNaR() || std::isnan(p_b.toDouble()) ||
            std::isnan(p_a.toDouble()) || std::abs(p_b.toDouble()) < 1e-6)
        {
            continue;
        }
...
TEST(Posit16Advanced, DistributiveProperty)
{
    ...
    std::uniform_int_distribution<int16_t> safe_dist(INT16_MIN / 4, INT16_MAX / 4);

    for (int i = 0; i < NTESTS16 / 20; i++)
    {  // Further reduced iteration count
        posit16 p_a, p_b, p_c;
        p_a.value = safe_dist(gen);
        p_b.value = safe_dist(gen);
        ...
        // Skip NaN/NaR values
        if (p_a.isNaR() || p_b.isNaR() || p_c.isNaR() || std::isnan(p_a.toDouble()) ||
            std::isnan(p_b.toDouble()) || std::isnan(p_c.toDouble()))
        {
            continue;
        }
        ...
        ASSERT_TRUE(double_eq(left_side.toDouble(), right_side.toDouble(), 1e-1))
            << "Distributive property failed: " << p_a.toDouble() << " * (" << p_b.toDouble()
            << " + " << p_c.toDouble() << ") = " << left_side.toDouble() << " but ("
            << p_a.toDouble() << " * " << p_b.toDouble() << " + " << p_a.toDouble() << " * "
            << p_c.toDouble() << ") = " << right_side.toDouble();
    }
}
...
TEST(Posit16Advanced, SubnormalHandling)
{
    ...
    // Should not be zero (posits don't have subnormals but have very small
    // values)
    ASSERT_NE(half_min.value, 0) << "Half of minpos became zero, expected a small positive value";
