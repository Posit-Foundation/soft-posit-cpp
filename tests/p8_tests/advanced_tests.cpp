...
TEST(Posit8Advanced, OverflowHandling)
{
    ...
    EXPECT_EQ(result.value, max_posit.value)
        << "Overflow did not saturate to maxpos: " << std::hex << "0x" << (int) result.value
    ...
    EXPECT_EQ(result.value, neg_max.value)
        << "Underflow did not saturate to -maxpos: " << std::hex << "0x" << (int) result.value
        << " vs 0x" << (int) neg_max.value;
}
...
TEST(Posit8Advanced, SubnormalHandling)
{
    ...
    // values)
    EXPECT_NE(half_min.value, 0) << "Half of minpos became zero, expected a small positive value";
